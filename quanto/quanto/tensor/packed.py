...
def pack_weights(intweights: torch.Tensor, bits: int) -> torch.Tensor:
    ...
    packed = torch.zeros(packed_tensor_shape, device=intweights.device, dtype=torch.int8)
    unpacked = intweights.to(torch.int8)
...
class PackedTensor(torch.Tensor):
    ...
    def __new__(cls, data, bits, size, stride, requires_grad=False):
        ...
        return torch.Tensor._make_wrapper_subclass(
            cls, size, strides=stride, dtype=torch.int8, device=data.device, requires_grad=requires_grad
        )
    ...
    def __torch_dispatch__(cls, op, types, args, kwargs=None):
        ...
        if op.overloadpacket is torch.ops.aten.detach:
            ...
            t = args[0]
            dtype = kwargs.get("dtype", torch.int8)
            if dtype != torch.int8:
